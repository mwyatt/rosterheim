...
export default function IndexPage({ params }) {
  ...
  const getCostRows = () => {
    ...
    formData['warriors'].map((warrior, index) => {
      ...
      for (let i = 0; i < warrior.qty; i++) {
        ...
        warrior['equipments'].map((equipmentName, index) => {
  ...
  const calculateRemainingGold = () => {
    ...
    const totalGold = costRows.reduce((total, costRow) => {
      return total + costRow.gold;
    }, 0);
          ...
          <div className={[
            ...
            <div className="border-b border-slate-300 mb-2 pb-2">
              ...
              <div>
                ...
                }>{inputData.costBreakdownOpen ? 'Hide' : 'Show'} Breakdown</span>

                <div>
                  {inputData.costBreakdownOpen && (
                      <>
                        <div>Starting Gold {startingGold}</div>
                        <div>Added Gold {formData.gold}</div>
